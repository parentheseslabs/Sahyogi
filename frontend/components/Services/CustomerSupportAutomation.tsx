...
import { CUSTOMER_SUPPORT_AUTOMATION_QUERY } from "../../lib/queries";
...
const CustomerSupportAutomation: React.FC = () => {
  const [serviceData, setServiceData] = useState<any>(null);
  const [isMobile, setIsMobile] = useState(false);
  const router = useRouter();

  const handleContactClick = () => {
    // Simple redirect to home page with contact hash
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const data = await client.fetch(CUSTOMER_SUPPORT_AUTOMATION_QUERY);
  ...
  const getDefaultData = () => ({
    ...
    subtitle: "Resolve queries faster, without burning out your team.",
    challenge: {
      icon: "⚠️",
      text: "Customers expect instant support, but SME teams are small."
    },
    solution: {
      icon: "💡",
      text: "Sahyogi's AI handles FAQs, order queries, and escalations instantly on WhatsApp."
    },
    features: [
    ...
    <section style={sectionStyle}>
      ...
      {serviceData.challenge && (
        <div style={{
          ...
          <div>
            <strong style={{ color: '#7f1d1d', fontSize: 'clamp(1rem, 2.5vw, 1.1rem)' }}>Challenge: </strong>
      ...
      {serviceData.solution && (
        <div style={{
          ...
          <div>
            <strong style={{ color: '#14532d', fontSize: 'clamp(1rem, 2.5vw, 1.1rem)' }}>Solution: </strong>
